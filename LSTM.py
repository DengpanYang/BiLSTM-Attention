...
# Load data
dataset = pd.read_csv('D:\\model and data\\MO155_M.csv',header=0, 
...
def evaluate_forecasts(actual, predicted):
    ...
    for row in range(actual.shape[0]):
        for col in range(actual.shape[1]):
            s += math.pow((actual[row, col] - predicted[row, col]),2)
...
# Set sliding-window
def sliding_window(train, sw_width=168, n_out=24, in_start=0):# set sliding-window
...
def Lstm_model(train, sw_width, in_start=0, verbose_set=0, epochs_num=20, batch_size_set=4):
    ...
    model.save('D:\\model and data\\LSTM\\LSTM-MO155')
...
def model_plot(history,epochs):
    ...
    df.to_csv('D:\\model and data\\LSTM\\LSTM-MO155_Mloss.csv')
...
def main_run(dataset,dataset2,sw_width, name, in_start, verbose, epochs, batch_size):
    ...
    df.to_csv('D:\\model and data\\LSTM\\LSTM-MO155_Mpred.csv')
    Y2=np.array(Y)
