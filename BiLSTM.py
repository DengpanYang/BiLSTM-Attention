...
# Load data
dataset = pd.read_csv('D:\\model and data\\PQ052_M.csv',header=0, 
...
def split_dataset(data,data2):#splitting data
...
def Bilstm_model(train, sw_width, in_start=0, verbose_set=0, epochs_num=20, batch_size_set=4):
    ...
    print(model.summary())
    model.save('D:\\model and data\\BiLSTM\\BiLSTM-PQ052')
...
def model_plot(history,epochs):
    ...
    df.to_csv('D:\\model and data\\BiLSTM\\BiL-PQ052_Mloss.csv')
...
def main_run(dataset,dataset2,sw_width, name, in_start, verbose, epochs, batch_size):
    ...
    df.to_csv('D:\\model and data\\BiLSTM\\BiL-PQ052_Mpred.csv')
    Y2=np.array(Y)
